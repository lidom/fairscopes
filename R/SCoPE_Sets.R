...
fairSCB_var <- function(alpha, hatvar,
  ...
  # Estimate the quantile funcion q.
  fair.q = NULL
  if(q.method$name == "KR_chisq"){
    if(type == "two-sided"){
      lb = fair_quantile_EEC_chisq(alpha = alpha/2,
                                  tau   = q.method$tau,
                                  x     = range(q.method$knots),
                                  df        = q.method$df,
                                  knots     = q.method$knots,
                                  ...
                                  type      = "linear",
                                  crossings = "up",
                                  alpha_up  = q.method$alpha_up/2,
                                  maxIter   = q.method$maxIter,
                                  tol = alpha / 100)$u
      ub = fair_quantile_EEC_chisq(alpha = alpha/2,
                                   tau   = q.method$tau,
                                   x     = range(q.method$knots),
                                   df        = q.method$df,
                                   knots     = q.method$knots,
      ...
      lb = fair_quantile_EEC_chisq(alpha = alpha,
                                   tau   = q.method$tau,
                                   x     = range(q.method$knots),
                                   df        = q.method$df,
                                   knots     = q.method$knots,
      ...
      SCB = cbind(q.method$df*hatvar/lb(x), rep(Inf, length(x)))
        ...
        ub = fair_quantile_EEC_chisq(alpha = alpha,
                                     tau   = q.method$tau,
                                     x     = range(q.method$knots),
                                     df        = q.method$df,
                                     knots     = q.method$knots,
