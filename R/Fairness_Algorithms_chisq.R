...
#' }
#' @export
alg1_KRF <- function(alpha, type, df = NULL, tau, knots,
  ...
  #       KacRice_chisq(tau,
  #                     u  = Vectorize(function(y){u_fun[1, k] + mk*(y - knots[k])}),
  #                     du = Vectorize(function(y){mk}),
  #                     crossings = crossings,
  #                     x  = c(knots[k], knots[k+1]),
  #                     df = df, t0 = 1) - alpha * I_weights[k]
  #     })
  #     u_fun[2, k] <- uniroot(f = find_mk, interval = c(-20, 20), extendInt = extendInt)$root
  #
    ...
    for(k in 2:ceiling(K/2)){
      ...
      find_mk <- function(mk){
        KacRice_chisq(tau,
                      ...
                      sigma = sigma, df = df, crossings = crossings, t0 = 1) - alpha * I_weights[2*k-2]
      }
      ...
      find_mk2 <- function(mk){
        KacRice_t(tau,
                         ...
                         sigma = sigma, df = df,
                         crossings = crossings, t0=2) - alpha * I_weights[2*k-1]
      }
