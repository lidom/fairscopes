...
#' }
#' @export
MultiplierBootstrapSplit_var <- function(R,
                                     plus,
                                     minus,
                                     Q = NULL,
                                     alpha   = 0.05,
  ...
  # Compute the sample varaince
  data.sigma <- t(t(matrixStats::rowVars( R ) ))
  ...
  bootSecMoments <- R^2 %*% multiplier^2 / N
  # We put an abs here to make sure that no NaNs are produced due to machine precision error.
  data.sigma.b <- ( N / ( N - 1 ) ) * abs( bootSecMoments - bootMeans^2 )

  # Compute bootstrap distribution of the maximum
  if(sum(plus) >= 1){
    distVec1 <- data.sigma.b[plus,] / (data.sigma[plus,] * (N-1))
  }else{
    distVec1 <- rep(-Inf, Mboots)
  }
  if(sum(minus) >= 1){
    distVec2 <- data.sigma.b[minus,] / (data.sigma[minus,] * (N-1))
  }else{
    distVec2 <- rep(-Inf, Mboots)
  }
...
find_quantiles <- function(X_samples, Y_samples, alpha = 0.1) {
  # Sortiere die Stichproben
  X_sorted <- sort(X_samples)
  Y_sorted <- sort(Y_samples)

  # Zielfunktion zur Optimierung
  objective <- function(p) {
    q_u <- quantile(X_samples, probs = 1 - p)  # (1 - p)-Quantil von X
    q_l <- quantile(Y_samples, probs = p)      # p-Quantil von Y

    # Schätze Wahrscheinlichkeiten empirisch
    P_X_gt_q_u <- mean(X_samples > q_u)
    P_Y_lt_q_l <- mean(Y_samples < q_l)
    P_joint <- mean((X_samples > q_u) & (Y_samples < q_l))

    # Fehlerfunktion: P_joint = alpha und P_X = P_Y
    return(abs(P_joint - alpha) + abs(P_X_gt_q_u - P_Y_lt_q_l))
  }

  # Optimierung: Finde p, sodass die Bedingungen erfüllt sind
  result <- optimize(objective, interval = c(0, 1))
  p_opt <- result$minimum

  # Berechne die finalen Quantile
  q_u_opt <- quantile(X_samples, probs = 1 - p_opt)
  q_l_opt <- quantile(Y_samples, probs = p_opt)

  return(list(q_u = q_u_opt, q_l = q_l_opt, p = p_opt))
}

# Beispielaufruf mit deinen Bootstrap-Samples:
# q_values <- find_quantiles(X_bootstrap, Y_bootstrap, alpha = 0.1)
# print(q_values)
